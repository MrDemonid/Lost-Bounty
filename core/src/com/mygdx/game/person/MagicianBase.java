...
public abstract class MagicianBase extends PersonBase {
    ...
    private static final int MANA_TO_HEAL = 2;  // коэфф. конвертации маны в здоровье
...
//        PersonBase p = friends.stream().filter(n -> n.health == 0).sorted((n1, n2) -> n2.priority - n1.priority).collect(Collectors.toList()).getFirst();
        ...
        if (p != null)
        {
            ...
            if (mana >= resurrectMana)
            {
                ...
                history.logWaitForMana(respawnTarget);
            }
        }
    }
    ...
    private void doResurrection(PersonBase person)
    {
        if (respawnTarget.getHealth() <= 0)
        {
            ...
            mana -= resurrectMana;
            history.logResurrect(respawnTarget);
        } else {
            history.logDeceasedTarget();
        }
        respawnTarget = null;
    }
...
//                Map.makeActionHealed(p.getPosition().getX(), p.getPosition().getY(), "" + (p.getHealth()-hp));
                history.logHealed(p, p.getHealth()-hp);
            }
        }
    }
